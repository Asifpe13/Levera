...
class AIService:
    ...
    def analyze_property(
        ...
        def _call() -> None:
            try:
                ...
                system_instruction = (
                    'אתה מנתח מודעות נדל"ן ישראלי. תפקידך הבלעדי הוא לבדוק אם מודעה עומדת בדרישות '
                    "הרוכש שסופקו לך — לא לשפוט את איכות העיר, פוטנציאל ההשקעה, או מדיניות כלכלית. "
                    "הרוכש בחר את הערים שלו בכוונה — זו לא אחריותך להעריך אותן. "
                    "תפקידך: (1) האם המודעה תיאר דירה אמיתית (לא מחסן/שותפים/הונאה)? "
                    "(2) האם הפרטים תואמים לדרישות? "
                    "ציון גבוה = מודעה אמינה שתואמת דרישות. ציון נמוך = מודעה חשודה/לא תואמת. "
    ...
    def _build_analysis_prompt(
        ...
        in_target = city in user_prefs.get('target_cities', [])

        return f"""בדוק את מודעת הנדל"ן הבאה וציין ציון התאמה מ-0 עד 100.

חוקי ניקוד מחייבים — חובה לפעול לפיהם:
• התחל מציון בסיס 70.
• הורד ניקוד רק אם המודעה חשודה/לא אמינה, או אם הפרטים אינם תואמים את הדרישות.
• אסור לנכות ניקוד על בסיס מיקום גיאוגרפי, עיר, או שיקולי השקעה — המשתמש בחר את הערים בכוונה.
• אסור להניח נתונים פיננסיים — {finance_note}

== פרטי המודעה ==
עיר: {city}{' ✅ (עיר מבוקשת על-ידי הרוכש)' if in_target else ''}
...
== דרישות הרוכש ==
ערים מבוקשות: {target_cities_str}
...
בדוק רק: (1) האם המודעה מתארת דירה אמיתית (לא מחסן/שותפים/הונאה)? (2) האם הפרטים תואמים לדרישות?
